...
static bool
comments( ifstream& in, string& name ) {
    for ( ;; ) {
        in >> name;
...
void
processConfigFile( const char* configFile, ConfigParms& cparms ) {
    enum { Parmnum = 10 };
    struct {
    ...
    try {
        ...
        for ( cnt = 0; cnt < Parmnum;
            ...
            in >> value;
            if ( value <= 0 ) {
                cerr << "Error: file \"" << configFile << "\" parameter "
                     << name << " value " << value << " must be positive."
                     << endl;
                exit( EXIT_FAILURE );
            }  // if
            if ( in.fail() ) break;
        ...
        cerr << "Error: could not open input file \"" << configFile << "\""
             << endl;
        exit( EXIT_FAILURE );
    }  // try
}  // processConfigFile
